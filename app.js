...
/* ============ 상태 ============
   ...
   - avoid  : 복수 선택이라 배열
   - name/tone/speech : STEP 5의 선택 입력(비워도 진행 가능)
   - speech : '반말' | '존댓말' — relation이 'boss'면 항상 '존댓말'로 강제됨 */
const state = { step:0, relation:null, age:null, style:null, budget:null, avoid:[], name:'', tone:'다정하게', speech:'반말' };
...
// STEP 1에서 관계를 고를 때마다(카드 클릭 버블링 이후) 존댓말 강제 여부를 재검사
const relationGridEl = document.getElementById('relationGrid');
if (relationGridEl) {
  relationGridEl.addEventListener('click', ()=>{
    updateSpeechConstraint();
  });
}
...
// STEP 5-2-1: 존댓말/반말 칩 (단일 선택, 기본값 '반말')
const speechRow = document.getElementById('speechRow');
if (speechRow) {
  // SPEECH_LEVELS가 data.js에 선언되어 있지 않더라도 안전하게 fallback 배열 사용
  const speechList = (typeof SPEECH_LEVELS !== 'undefined') ? SPEECH_LEVELS : ['반말', '존댓말'];
  speechRow.innerHTML = speechList.map((s,i)=>`<button class="chip big${i===0?' sel':''}" data-key="${s}">${s}</button>`).join('');
  speechRow.querySelectorAll('.chip').forEach(chip=>{
    chip.addEventListener('click', ()=>{
      if(chip.disabled) return; // boss 관계일 때 반말 버튼은 클릭 무시
      speechRow.querySelectorAll('.chip').forEach(c=>c.classList.remove('sel'));
      chip.classList.add('sel');
      state.speech = chip.dataset.key;
    });
  });
}

/**
 * 관계(state.relation)가 '직장 상사 · 어려운 동료'(boss)이면
 * STEP 5의 반말 버튼을 비활성화하고 존댓말로 강제 고정한다.
 * boss가 아니면 다시 반말/존댓말 둘 다 선택 가능하게 풀어준다.
 */
function updateSpeechConstraint(){
  if (!speechRow) return;
  const forced = state.relation === 'boss';
  speechRow.querySelectorAll('.chip').forEach(chip=>{
    const isInformal = chip.dataset.key === '반말';
    chip.disabled = forced && isInformal;
    chip.classList.toggle('disabled', forced && isInformal);
  });
  if (forced) {
    state.speech = '존댓말';
    speechRow.querySelectorAll('.chip').forEach(c=>c.classList.toggle('sel', c.dataset.key==='존댓말'));
  }
}
// 최초 로드 시에도 한 번 검사
updateSpeechConstraint();
...
if (nameInputEl) {
  nameInputEl.addEventListener('input', e=>{ state.name = e.target.value.trim(); });
}
...
function goTo(step){
  ...
  if (dotsEl) dotsEl.style.visibility = (step>=1 && step<=5) ? 'visible' : 'hidden';
  
  // 하단 이전/다음 버튼바는 인트로(0)·로딩(6)·결과(7)에서는 숨김
  if (bottombar) bottombar.classList.toggle('hidden', step===0 || step===6 || step===7);
...
if (startBtnEl) startBtnEl.addEventListener('click', ()=>goTo(1));
...
if (restartBtnEl) {
  restartBtnEl.addEventListener('click', ()=>{
    state.relation=null; state.age=null; state.style=null; state.budget=null; state.avoid=[]; state.name=''; state.tone='다정하게'; state.speech='반말';
    feedbackCount = 0;
    document.querySelectorAll('.choice-card.sel').forEach(c=>c.classList.remove('sel'));
    if (budgetRow) budgetRow.querySelectorAll('.chip.sel').forEach(c=>c.classList.remove('sel'));
    if (avoidRow) avoidRow.querySelectorAll('.chip.sel').forEach(c=>c.classList.remove('sel'));
    if (toneRow) toneRow.querySelectorAll('.chip').forEach((c,i)=>c.classList.toggle('sel', i===0));
    if (speechRow) {
      speechRow.querySelectorAll('.chip').forEach((c,i)=>{
